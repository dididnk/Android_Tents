...
game game_copy(cgame g) {
  game gg = game_new_ext(g->nb_rows, g->nb_cols, g->squares, g->nb_tents_row,
                         g->nb_tents_col, g->wrapping, g->diagadj);
  return gg;
}
...
bool game_is_over(cgame g) {
  ...
  for (uint i = 0; i < g->nb_rows; i++)
    for (uint j = 0; j < g->nb_cols; j++)
      if ((SQUARE(g, i, j) == TENT) && _neigh(g, i, j, TENT, !g->diagadj))
        return false;
  ...
  for (uint i = 0; i < g->nb_rows; i++)
    for (uint j = 0; j < g->nb_cols; j++) {
      if ((SQUARE(g, i, j) == TENT) && !_neigh(g, i, j, TREE, false))
        return false;
      if ((SQUARE(g, i, j) == TREE) && !_neigh(g, i, j, TENT, false))
        return false;
    }
